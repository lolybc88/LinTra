...
public class MTLauncher1Input1OutputDistributed_Machine1 {
	...
	IArea workTODOArea, workTODOLocalArea, srcModelArea, trgModelArea;
...
//		workTODOArea = blackboard.createLocalArea("processorSpace", Policy.LOCK_TO_READ);
		workTODOLocalArea = blackboard.createLocalArea("processorSpace2", Policy.LOCK_TO_READ);
		srcModelArea = blackboard.createArea("processorSpace_Src", Policy.NEVER_LOCK);
		trgModelArea = blackboard.createLocalArea("processorSpace_Trg", Policy.NEVER_LOCK);
...
//		((DistributedArea_Machine1)trgModelArea).connectToRemoteSubArea(remoteAreaIP, trgModelAreaPort_M2);
	...
	public double launch(ITransformation transfo, int numThreads, double cutId) throws Exception{
		
		double maxId = srcModelArea.size();
		
		IMaster master = new Master_SingleMT_Machine1(workTODOLocalArea, srcModelArea, numThreads);
		((Master_SingleMT_Machine1)master).organizeWork(srcModelArea, cutId, maxId);
				
		double time0 = System.currentTimeMillis();
		
		List<Thread> ts = new LinkedList<Thread>();
		for (int j=0; j<numThreads; j++){
			Thread t = new Thread(new Slave_SingleMT_machine1(j, transfo, workTODOLocalArea, srcModelArea));
			t.start();
			ts.add(t);
    	}
