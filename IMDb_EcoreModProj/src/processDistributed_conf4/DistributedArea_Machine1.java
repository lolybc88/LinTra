...
public class DistributedArea_Machine1 implements IArea {
	...
	public DistributedArea_Machine1(String name, Policy p) throws BlackboardException {
		this.name = name;
		this.policy = p;
...
//		out.addAll(readAllFromRemoteArea(remoteIds));
		...
		out.addAll(readAllFromLocalArea(ids));
		return out;
		
	}
		
	public synchronized Collection<IdentifiableElement> readAllFromLocalArea(Collection<String> ids) {	
		Collection<IdentifiableElement> out = new LinkedList<IdentifiableElement>();
		for(String id : ids){
	...
	public Collection<? extends IdentifiableElement> readAllFromRemoteArea(
	...
	private synchronized void writeAux(IdentifiableElement elem, int machine)
	...
	private synchronized void writeIntoLocalArea(IdentifiableElement elem) throws BlackboardException {	
		localSubArea.put(elem.getId(), elem);	
	}
	
	private synchronized void writeIntoRemoteArea(IdentifiableElement elem) throws BlackboardException {	
		try {
...
//			}
	...
	public synchronized void writeAllIntoLocalArea(Collection<IdentifiableElement> elems) {
		for (IdentifiableElement e : elems){
			try {
...
//					System.out.println("--->"+area.get(e.getId()) + " is being overwritten by " + e);
	...
	public void writeAllIntoRemoteArea(Collection<IdentifiableElement> remoteElems) throws BlackboardException {
		try {
...
//			}
	...
	public synchronized boolean equals(Object o){
		return o instanceof DistributedArea_Machine1 && ((DistributedArea_Machine1) o).getName().equals(name);		
	}
