...
//		((DistributedArea_Machine2)trgModelArea).connectToRemoteSubArea(remoteAreaIP, trgModelAreaPort_M1);
	...
	public double launch(ITransformation transfo, int numThreads, double cutId) throws Exception{
		...
		IMaster master = new Master_SingleMT_Machine2(workTODOArea, srcModelArea, numThreads);
		((Master_SingleMT_Machine2)master).organizeWork(srcModelArea, cutId, maxId);
		
		double time0 = System.currentTimeMillis();
		
		List<Thread> ts = new LinkedList<Thread>();
		for (int j=0; j<numThreads; j++){
			Thread t = new Thread(new Slave_SingleMT_machine2(j, transfo, workTODOArea, srcModelArea));
			t.start();
			ts.add(t);
    	}
