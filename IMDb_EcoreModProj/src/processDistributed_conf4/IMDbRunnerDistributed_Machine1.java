...
public class IMDbRunnerDistributed_Machine1 {

	public static void main(String[] args) throws Exception {
		
		MTLauncher1Input1OutputDistributed_Machine1 mtLauncher =
				new MTLauncher1Input1OutputDistributed_Machine1("localhost", 9895, 9894, 9893, 9892, 9896, 9897);
		// 192.168.1.4
		...
		int n = 100;
		createSyntheticData(n, mtLauncher.getSrcArea());
...
//	
	public static void createSyntheticData(int n, IArea area) throws BlackboardException{
		...
		for(int i=0; i<n; i++){
			try {
				if (i<(n*20)/2){
					createPoSitive(area, i, true); 
					createNegative(area, i, true);
				} else {
					createPoSitive(area, i, false); 
					createNegative(area, i, false);
				}
	...
	private static void createNegative(IArea area, int i, boolean local) throws BlackboardException {
		...
		if (local){
			((DistributedArea_Machine1)area).writeAllIntoLocalArea(elems);
		} else {
			((DistributedArea_Machine1)area).writeAllIntoRemoteArea(elems);
		}		
	}

	private static void createPoSitive(IArea area, int i, boolean local) throws BlackboardException {
