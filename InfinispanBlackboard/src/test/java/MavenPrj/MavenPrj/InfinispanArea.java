...
public class InfinispanArea implements IArea {
	...
	public InfinispanArea(String name, Policy p, EmbeddedCacheManager manager) throws IOException {
	...
	public boolean write(IdentifiableElement elem) throws BlackboardException {
		try {
			if (policy.equals(Policy.ALWAYS_LOCK) || policy.equals(Policy.LOCK_TO_WRITE)){
				semaphore.acquire();
				area.getAdvancedCache().withFlags(Flag.SKIP_CACHE_LOAD).put(elem.getId(), elem);
				semaphore.release();
			} else {
				area.getAdvancedCache().withFlags(Flag.SKIP_CACHE_LOAD).put(elem.getId(), elem);
			}
