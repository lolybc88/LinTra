...
public class DeleteAndDeleteLinks_Semantic1 implements ITransformation {

	private static final String A2AC = "a2ac";
	IArea srcArea, trgArea, currentIdArea, idCorrespondencesArea, deletesArea;
	
	public DeleteAndDeleteLinks_Semantic1(IArea srcArea, IArea trgArea, IArea currentIdArea, IArea correspondencesArea, IArea deletesArea) {
	...
	public void transform(Collection<IdentifiableElement> objs,
		...
		for (IdentifiableElement ie : objs){
			if (ie instanceof A){
				String refToC = null;
				A a = new A(ie.getId(), TraceFunction.create(ie.getId(), 1, A2AC), ((A) ie).getName(), refToC);
				modifiedElems.add(a);
